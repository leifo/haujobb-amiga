...
static const char *sync_track_path(const char *base, const char *name)
{
   ...
   strncat(temp, base, sizeof(temp) - 1);
	...
	strncat(temp, ".rt", sizeof(temp) - strlen(temp) - 1);
	return temp;
}
...
static int read_track_data(struct sync_device *d, struct sync_track *t)
{
   ...
   filename= sync_track_path(d->base, t->name);
   //printf("read track data from '%s' to %08x \n", filename, t);
   streamInit(&stream, filename);

   t->num_keys= streamReadInt(&stream);
   ...
   for (i = 0; i < (int)t->num_keys; ++i)
   {
      ...
      key->row= streamReadInt(&stream);
      key->value= streamReadFloat(&stream);
      key->type = (enum key_type)streamReadByte(&stream);
   }
...
static int create_track(struct sync_device *d, const char *name)
{
   ...
   assert(find_track(d, name) < 0);
	...
	d->num_tracks++;
   oldtracks= d->tracks;
   d->tracks = (struct sync_track**)malloc(sizeof(struct sync_track*) * d->num_tracks);
   if (oldtracks)
   {
      memcpy(d->tracks, oldtracks, sizeof(struct sync_track*) * (d->num_tracks-1));
      free(oldtracks);
   }
